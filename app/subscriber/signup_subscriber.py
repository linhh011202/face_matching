...
class SignUpSubscriber:
    """
    Pull messages from the PubSub subscription for sign-up events.
    ...
    On receiving a message:
      1. Parse the user_id from the message
    ...
    def __init__(self, face_processing_service: FaceProcessingService) -> None:
        ...
        logger.info(
            f"SignUpSubscriber initialized — subscription: {self._subscription_path}"
        )
    ...
    def _handle_message(self, message: pubsub_v1.subscriber.message.Message) -> None:
        ...
        try:
            ...
            logger.info(
                f"Processing sign_up event for user_id: {user_id} (session_id={session_id})"
            )
            success = self._face_processing_svc.process_user(user_id)
    ...
    def start(self) -> None:
        ...
        logger.info(f"Starting SignUpSubscriber on {self._subscription_path}...")
        ...
        logger.info("SignUpSubscriber is running. Waiting for messages...")
        print("=" * 60)
        print("  Face Matching Worker — Sign Up")
    ...
    def stop(self) -> None:
        ...
        if self._streaming_pull_future:
            self._streaming_pull_future.cancel()
            logger.info("SignUpSubscriber stopped")
